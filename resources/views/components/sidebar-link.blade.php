...
<a href="{{ $href }}" {{ $attributes->merge([
    ...
    <svg class="w-5 h-5 flex-shrink-0 {{ $active ? 'text-blue-400' : 'text-gray-500 group-hover:text-blue-400' }}"
        fill="none" stroke="currentColor" viewBox="0 0 24 24">
        {!! $iconPath !!}
    </svg>
    <span x-show="sidebarOpen" x-cloak class="text-sm font-medium">{{ $slot }}</span>
</a>