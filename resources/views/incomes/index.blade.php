<x-layouts.dashboard>
    ...
    <div x-data="{ activeTab: 'summary' }" class="mb-6 bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-5 shadow-xl">
        ...
        <div class="flex items-center justify-between mb-4 pb-3 border-b border-gray-700/50">
            <div class="flex items-center gap-2">
                <button @click="activeTab = 'summary'" 
                        :class="activeTab === 'summary' ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'text-gray-400 hover:text-gray-200 border-transparent'"
                ...
                <button @click="activeTab = 'chart'" 
                        :class="activeTab === 'chart' ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'text-gray-400 hover:text-gray-200 border-transparent'"
                        class="px-3 py-1.5 rounded-xl text-sm font-medium border transition-all flex items-center gap-2">
            ...
            <a href="{{ route('incomes.create') }}"
                class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 rounded-xl text-sm font-medium transition-all shadow-lg shadow-green-500/25">
        ...
        <div x-show="activeTab === 'summary'" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 -translate-x-4" x-transition:enter-end="opacity-100 translate-x-0" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="bg-gray-700/30 rounded-2xl p-4 border border-gray-700/50">
                ...
                <p class="text-2xl font-bold text-white">Rp {{ number_format($averageIncome ?? 0, 0, ',', '.') }}</p>
            </div>
        </div>

        <!-- Slide 2: Grafik Tren Pemasukan -->
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
            ...
            <div class="md:col-span-4">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari pemasukan..."
                    class="w-full px-4 py-2 bg-gray-700/50 border border-gray-600 rounded-xl text-sm focus:ring-2 focus:ring-green-500/50 focus:border-green-500 transition text-gray-200 placeholder-gray-400">
            </div>
            
            <!-- Category Dropdown -->
            <div class="md:col-span-3">
                <select name="category"
                    class="w-full px-4 py-2 bg-gray-700/50 border border-gray-600 rounded-xl text-sm focus:ring-2 focus:ring-green-500/50 focus:border-green-500 transition text-gray-200">
            ...
            <div class="md:col-span-5 flex items-center gap-1.5 bg-gray-700/50 p-1 rounded-xl border border-gray-600">
                ...
                <!-- Tombol Filter -->
                <button type="submit" 
                    class="px-3 py-1.5 bg-green-600 hover:bg-green-500 text-white font-medium rounded-lg text-sm transition flex items-center gap-1 shrink-0 shadow-md shadow-green-600/20">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <tbody class="divide-y divide-gray-700/50">
                    @forelse($incomes as $income)
                        <tr class="hover:bg-gray-700/30 transition">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{{ $income->date->format('d M Y') }}</td>
                            <td class="px-6 py-4">
                                <p class="font-medium text-gray-200">{{ $income->title }}</p>
                                @if($income->notes)
                                    <p class="text-sm text-gray-400 truncate max-w-xs">{{ $income->notes }}</p>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-2.5 py-1 rounded-lg bg-green-500/20 text-green-400 text-sm font-medium">{{ ucfirst($income->category) }}</span>
                            </td>
                            <td class="px-6 py-4 text-right font-semibold text-green-400">Rp {{ number_format($income->amount, 0, ',', '.') }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('incomes.edit', $income) }}" class="p-2 rounded-lg hover:bg-gray-600 transition">
                                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </a>
                                    <form action="{{ route('incomes.destroy', $income) }}" method="POST" onsubmit="return confirm('Hapus pemasukan ini?')">
                                        @csrf
                                        @method('DELETE')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Dummy Data (Ganti dengan data dinamis dari Controller)
