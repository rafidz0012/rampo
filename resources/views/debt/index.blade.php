<x-layouts.dashboard>
    ...
    <div x-data="{ activeTab: 'summary' }" class="mb-6 bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-5 shadow-xl">
        ...
        <div x-show="activeTab === 'summary'" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 -translate-x-4" x-transition:enter-end="opacity-100 translate-x-0" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="bg-gray-700/30 rounded-2xl p-4 border border-gray-700/50">
                <p class="text-sm text-gray-400">Total Akumulasi Hutang</p>
                <p class="text-2xl font-bold text-white">Rp {{ number_format($totalAllDebts ?? 0, 0, ',', '.') }}</p>
            </div>
        </div>

        <!-- Slide 2: Grafik Tren Hutang -->
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
            ...
            <div class="md:col-span-3">
                <select name="status"
                    ...
                    <option value="pending" {{ request('status') === 'pending' ? 'selected' : '' }} class="bg-gray-800 text-gray-200">Belum Dibayar</option>
                ...
                <tbody class="divide-y divide-gray-700/50">
                    @forelse($debts as $debt)
                        <tr class="hover:bg-gray-700/30 transition">
                            <td class="px-6 py-4">
                                <p class="font-semibold text-gray-200">{{ $debt->title }}</p>
                            ...
                            <td class="px-6 py-4 text-center">
                                ...
                                @elseif($debt->status === 'partial')
                                    <span class="px-2.5 py-1 rounded-lg bg-yellow-500/20 text-yellow-400 text-xs font-semibold">Dicicil</span>
                                @else
                                    <span class="px-2.5 py-1 rounded-lg bg-red-500/20 text-red-400 text-xs font-semibold">Belum Dibayar</span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('debt.edit', $debt) }}" class="p-2 rounded-lg hover:bg-gray-600 transition" title="Edit">
                                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </a>
                                    <form action="{{ route('debt.destroy', $debt) }}" method="POST" onsubmit="return confirm('Hapus data hutang ini?')">
                                        @csrf
                                        @method('DELETE')
